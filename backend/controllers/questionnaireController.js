...
export const createQuestionnaire = async (req, res) => {
  try {
    ...
    // Normalize stressLevel values from frontend (acceptable: Low/Moderate/High)
    if (questionnaireData && questionnaireData.stressLevel) {
      const map = {
        'low': 'Low',
        'moderate': 'Moderate',
        'high': 'High',
      ...
      const key = questionnaireData.stressLevel.toString().trim().toLowerCase();
      if (map[key]) questionnaireData.stressLevel = map[key];
    }
    ...
    if (questionnaire) {
      ...
      questionnaire = await Questionnaire.findByIdAndUpdate(
        questionnaire._id,
        { ...questionnaireData, isCompleted: true },
...
export const updateQuestionnaire = async (req, res) => {
  try {
    ...
    if (updateData.age || updateData.gender) {
      await User.findByIdAndUpdate(userId, {
        age: updateData.age || questionnaire.age,
        gender: updateData.gender || questionnaire.gender
      });
    }
